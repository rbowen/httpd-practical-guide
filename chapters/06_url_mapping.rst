...
When Apache HTTP Server receives a request, it is assumed that the client
...
.. admonition:: Modules covered in this chapter
   ...
   :module:`mod_alias`, :module:`mod_dir`, :module:`mod_rewrite`,
   :module:`mod_speling`
...
Use a ``<Directory>`` block to grant access:
...
A request for http://example.com/icons/test.gif results in httpd
...
site. You want to redirect a number of URLs to the same place. For
example, you want to redirect requests for **/fish** and **/Fishing** to http://fish.example.com/.
...
You want requested URLs to be valid whether uppercase or
...
part of the standard httpd distribution but is not enabled by
...
statement - that is, ``support`` vs ``Support``, you could redirect just
that one URL. But ``mod_rewrite`` solves the larger case where upper-
and lower-case letters have been used indiscriminately in URLs, and you
